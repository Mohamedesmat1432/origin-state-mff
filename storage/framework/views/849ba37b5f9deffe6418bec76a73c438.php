...
<div x-data="{ show: <?php if ((object) ($attributes->wire('model')) instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e($attributes->wire('model')->value()); ?>')<?php echo e($attributes->wire('model')->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e($attributes->wire('model')); ?>')<?php endif; ?> }" x-on:close.stop="show = false"
    x-on:keydown.escape.window="show = false" x-show="show" id="<?php echo e($id); ?>"
    class="jetstream-modal fixed inset-0 overflow-y-auto px-4 py-6 sm:px-0 z-50" style="display: none;">

    <div x-show="show" wire:click="$dispatch('close-modal')" x-on:click="show = false"
        class="fixed inset-0 transform transition-all" x-transition:enter="ease-out duration-300"
    ...
    <div x-show="show"
        class="my-8 bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:w-full <?php echo e($maxWidth); ?> sm:mx-auto"
...
</div><?php /**PATH /var/www/resources/views/components/modal.blade.php ENDPATH**/ ?>