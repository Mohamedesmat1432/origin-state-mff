<div>
    @can('edit-origin')
    <x-dialog-modal wire:model="edit_modal" submit="save()" method="PATCH" max-width="5xl">
        @if ($this->edit_modal)
        <x-slot name="title">
        ...
        <x-slot name="content">
            ...
            <div class="mt-2">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 items-center">
                    ...
                    <x-form.field label="{{ __('site.number_of_units') }}" model="details.{{ $index }}.number_of_units"
                        type="number" placeholder="{{ __('site.number_of_units') }}" />
                    ...
                    <div class="col-span-2 md:col-span-4">
                        <x-form.field label="{{ __('site.notes') }}" model="details.{{ $index }}.note" type="textarea"
                            placeholder="{{ __('site.notes') }}" class="w-full" />
                    </div>

                    {{-- Delete button - centered --}}
                    <div class="col-span-2 md:col-span-4 flex justify-center">
                        <x-danger-button class="p-2 flex items-center gap-1" wire:click="removeDetail({{ $index }})"
            ...
            <div class="mt-2">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4 items-center">
                    ...
                    {{-- Notes - full width --}}
                    <div class="col-span-2 md:col-span-4">
                        <x-form.field label="{{ __('site.notes') }}" model="services.{{ $index }}.note" type="textarea"
                            placeholder="{{ __('site.notes') }}" class="w-full" />
                    </div>

                    {{-- Delete button - centered --}}
                    <div class="col-span-2 md:col-span-4 flex justify-center">
                        <x-danger-button class="p-2 flex items-center gap-1" wire:click="removeService({{ $index }})"
                            wire:loading.attr="disabled">
                            <x-icon class="w-4 h-4" name="trash" />
                            {{ __('site.remove') }}
                        </x-danger-button>
